...
class DockerServiceManager:
    ...
    def check_service_status(self) -> Tuple[bool, Optional[str]]:
    ...
    def show_docker_info(self):
        ...
        try:
            if self.demo_mode:
                print("[demo] Showing Docker information in demo mode")
                return True, "Demo mode: Docker information would be displayed here"
...
API Version: {info.get('ApiVersion', 'N/A')}
Go Version: {info.get('GoVersion', 'N/A')}
OS/Arch: {info.get('OperatingSystem', 'N/A')}/{info.get('Architecture', 'N/A')}
Kernel Version: {info.get('KernelVersion', 'N/A')}

Resources:
- CPUs: {info.get('NCPU', 'N/A')}
- Total Memory: {info.get('MemTotal', 'N/A')} bytes
...
Docker Root Dir: {info.get('DockerRootDir', 'N/A')}
Storage Driver: {info.get('Driver', 'N/A')}
            """
            
            print(report)
            return True, "Docker information displayed successfully"
            
        except Exception as e:
            return False, f"Error showing Docker information: {str(e)}"